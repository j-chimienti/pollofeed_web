<template>
  <q-markup-table  v-if="INVOICES.length">
    ...
    <tr v-for="invoice in sortedInvoices()" :key="invoice.id">
      ...
      <td>
        <div class="badge rounded" :class="{
          'badge-success': invoice.status === 'complete' || invoice.status === 'confirmed',
         'badge-default': invoice.status === 'new'

        }">{{ invoice.status }}</div>
      </td>
      <td class="text-center">
        <i v-if="invoice.sentShippingEmail" class="fa fa-circle text-success"></i>
...
export default {
  ...
  computed: {
    ...
    sum() {
      return this.INVOICES.map(i => i.items.map(it => it.price).reduce((a, b) => a + b, 0)).reduce((a, b) => a + b, 0)
    },
  },
  methods: {
    sortedInvoices() {
